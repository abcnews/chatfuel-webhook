...
const slackModule = {
  ...
  render(config, values) {
    ...
    // Author link to dashbot, or avatar if dashbot not configured.
    const dashbotUserId = process.env.DASHBOT_USERID;
    let authorLink = values['profile pic url'];
    if (dashbotUserId) {
      authorLink = `https://www.dashbot.io/live/session/${dashbotUserId}/${qs['messenger user id']}`;
    }
    ...
    const shortFields = [
      ['City', qs.country ? `${qs.city || 'unknown city'}, ${qs.country}` : '-'],
      ['Locale', qs.locale || '-'],
      ['Gender', qs.gender || '-'],
